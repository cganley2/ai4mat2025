...
train_set_path = '/scratch/cganley2/ran1500/ran-train1500.extxyz'
valid_set_path = '/scratch/cganley2/ran1500/ran-valid1500.extxyz'
test_set_path = '/scratch/cganley2/ran1500/ran-test13500.extxyz'
epochs = 100
...
checkpoints = os.listdir("/scratch/cganley2/ran1500/training/model_training/model")
...
loaded_params_via_ckpt = load_parameters_from_checkpoint(
    local_checkpoint_dir=Path("/scratch/cganley2/ran1500/training/model_training/model").resolve(),
...
train_structures = ase_read(train_set_path, index=':')
valid_structures = ase_read(valid_set_path, index=':')

predictions = run_batched_inference(test_structures, loaded_force_field, batch_size=50)
predictions_train = run_batched_inference(train_structures, loaded_force_field, batch_size=50)
predictions_valid = run_batched_inference(valid_structures, loaded_force_field, batch_size=50)
...
parity_df_train = pd.DataFrame({'DFT Energy (eV)': [train_structures[i].get_potential_energy() for i in range(len(train_structures))],
                          'MACE Energy Prediction (eV)': [predictions_train[i].energy for i in range(len(predictions_train))]})

parity_df_valid = pd.DataFrame({'DFT Energy (eV)': [valid_structures[i].get_potential_energy() for i in range(len(valid_structures))],
                          'MACE Energy Prediction (eV)': [predictions_valid[i].energy for i in range(len(predictions_valid))]})


# Plotting helper
def parity_plot(ax, x, y, title):
    ax.plot([x.min(), x.max()], [x.min(), x.max()], 'r--')
    ax.scatter(x, y, alpha=0.6, edgecolors='w', linewidth=0.5, s=80)#, label='Test')
    #ax.plot([x.min(), x.max()], [x.min(), x.max()], 'r--')
    ax.set_xlabel('DFT',fontsize=24)
    ax.set_ylabel('MACE Prediction',fontsize=24)
...
energy_parity_figure, energy_parity_axes = plt.subplots(figsize=(16,12))
parity_plot(energy_parity_axes, parity_df['DFT Energy (eV)'], parity_df['MACE Energy Prediction (eV)'], 'Energy Parity (eV)')

energy_parity_axes.scatter(parity_df_train['DFT Energy (eV)'], parity_df_train['MACE Energy Prediction (eV)'], marker='*', alpha=0.8, edgecolors='w', linewidth=0.5, s=80)
#energy_parity_axes.scatter(parity_df_valid['DFT Energy (eV)'], parity_df_valid['MACE Energy Prediction (eV)'], marker='+', label='Validation')

energy_parity_axes.legend(['y = x', 'Test', 'Training'], markerscale=2)


with open('train-test-split-energy-parity-plot.pkl', 'wb') as f:
   pickle.dump((energy_parity_figure, energy_parity_axes), f)

energy_parity_figure.savefig('train-test-split-energy-parity-plot.png', dpi=600)
