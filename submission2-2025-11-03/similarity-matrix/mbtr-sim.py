...
def vary_nframes(nframes_list):
    for j in nframes_list:
        ...
        traj500 = ase.io.read('/scratch/cganley2/mlip/Maximize_Diversity_using_RL_Dataset/train-set-300.extxyz', index=':')
        i = len(traj500)
...
def vary_gamma(gamma_list):
    for j in gamma_list:
        # i = 5000-j
        traj500 = ase.io.read('/home/cganley2/ai4mat2025/submission2/mace-models/rl1500/rl-train1500.extxyz', index=':')
        ...
        with open('MBTR-RBF-rl-{0}gamma-1500frames-sim-matrix.pkl'.format(gamma), 'wb') as f:
            pickle.dump(mbtr_sim, f)
        ...
        sns.histplot(values, bins=30, kde=False) # Histogram with density
        ax2.set_xlabel('Similarity Score', fontsize=20)
        ax2.set_title(r'Similarity Distribution: $\gamma$={0}'.format(gamma), fontsize=20)
        ax2.set_yticklabels([])
        ax2.set_ylabel('Count', fontsize=20)
        # ax2.text(0.35, 6000, 'VS = {0}'.format(vs), fontsize=18)
        ax2.tick_params(axis='x', labelsize=20)
        ...
        # plt.tight_layout()
        # plt.show()
        fig.savefig('MBTR-RBF-rl-{0}gamma-Aheatmap-Bhist.png'.format(gamma), dpi=400) #, bbox_inches='tight')

        with open('MBTR-RBF-rl-{0}gamma-Aheatmap-Bhist.pkl'.format(gamma), 'wb') as f:
            pickle.dump(fig, f)

if __name__ == '__main__':
    # vary_nframes([300])
    vary_gamma([5e3, 1e4, 5e4])