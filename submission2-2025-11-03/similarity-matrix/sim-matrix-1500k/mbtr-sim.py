...
def vary_gamma(gamma_list):
    for j in gamma_list:
        # i = 5000-j
        traj500 = ase.io.read('SiC_3C-aimd-1500K.out', index=':')
        ...
        with open('MBTR-RBF-{0}gamma-15000frames-sim-matrix.pkl'.format(gamma), 'wb') as f:
            pickle.dump(mbtr_sim, f)
        ...
        # plt.tight_layout()
        # plt.show()
        fig.savefig('MBTR-RBF-{0}gamma-Aheatmap-Bhist.png'.format(gamma), dpi=400) #, bbox_inches='tight')

        with open('MBTR-RBF-{0}gamma-Aheatmap-Bhist.pkl'.format(gamma), 'wb') as f:
            pickle.dump(fig, f)

if __name__ == '__main__':
    # vary_nframes([300])
    vary_gamma([5e5, 1e6, 5e6])
